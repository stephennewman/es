...
export default function BinaryChoice({
  ...
  if (options.length !== 2) {
    console.error('BinaryChoice requires exactly 2 options');
    return null;
  }
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {options.map((option, index) => (
          <label
            key={index}
            className="flex items-start space-x-3 p-4 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors border-2 border-gray-200 dark:border-gray-700 data-[checked=true]:border-black dark:data-[checked=true]:border-white"
            data-checked={String(value) === String(index)}
          >
            <input
              type="radio"
              name={questionId}
